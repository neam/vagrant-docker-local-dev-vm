...
# Remove any containers that failed to start (if they are not removed, vagrant won't attempt to start a new working container)
    ...
    docker ps -a | grep '_api_' | grep 'Exited' | awk '{ print $1 }' | xargs docker rm -f
    docker ps -a | grep '_external' | grep 'Exited' | awk '{ print $1 }' | xargs docker rm -f
    docker ps -a | grep '_internal' | grep 'Exited' | awk '{ print $1 }' | xargs docker rm -f
